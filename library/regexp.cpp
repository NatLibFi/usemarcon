...
TCHAR* CRegExp::GetReplaceString( const TCHAR* sReplaceExp )
{
    ...
    while ((c = *src++) != _T('\0'))
    {
        ...
        else if (no < NSUBEXP && startp[no] != NULL && endp[no] != NULL &&
                    endp[no] > startp[no])
        {
            // Get tagged expression
            len = endp[no] - startp[no];
...
// GetMatch - Returns one of the matches a previous call to RegFind found
// Returns          - Pointer to newly allocated string
//                    Caller is responsible for deleting it
TCHAR* CRegExp::GetMatch(int index)
{
    if (index >= NSUBEXP || startp[index] == NULL || endp[index] == NULL || sFoundText == NULL)
        return NULL;

    size_t len = endp[index] - startp[index];
    TCHAR* buf = new TCHAR[len + 1]; 
    if (len > 0)
    {
        int tagpos = startp[index] - startp[0];
        _tcsncpy(buf, sFoundText + tagpos, len);
    }
    buf[len] = _T('\0');
    return buf;
